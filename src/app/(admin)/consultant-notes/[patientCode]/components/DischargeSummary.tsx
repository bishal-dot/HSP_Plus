...
const EMPTY_FORM: DischargeFormData = {
  IPDCode: "", DischargeType: "", PreOPDiagnosis: "", Diagnosis: "",
...
/* ═══════════════════════════════════════════════════
   SUMMARY VIEWER  (unchanged from before)
═══════════════════════════════════════════════════ */
...
const IPDDischargeForm: React.FC<{ MrNO: string; onSuccess: () => void }> = ({ MrNO, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    if (!authToken) return;
    try {
      await insertDischarge({ authToken, MrNO, ...form });
      onSuccess();
    } catch (_) {
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="IPD Code" required>
            <input
              type="text"
              placeholder="e.g. IPD-2024-001"
              value={form.IPDCode}
              onChange={set("IPDCode")}
              className={inputCls}
            />
          </Field>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            ...
            disabled={isPending || !form.IPDCode || !form.DischargeType || !form.Diagnosis}
            className="
...
function Accordion({ icon, title, color = "blue", open, onToggle, children }: {
