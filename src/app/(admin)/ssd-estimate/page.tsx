            ...
            <div className="min-w-[275.5px] min-h-[45vh] max-h-[70vh]">
              <Table>
                <TableHeader className="border-b border-gray-100 dark:border-white/5 sticky top-0 bg-gray-50 dark:bg-gray-800 z-10">
                  <TableRow>
                    {["S.No", "Patient Name", "Patient Code", "Age/ Sex", "Amount", "Status / Amount", "Action"].map((header) => (
                      <TableCell
                        key={header}
                        isHeader
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/5">
                  {patientInfo && patientInfo.map((refferred, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className="px-3 py-3 text-sm text-gray-700 dark:text-gray-300">
                        {new Intl.NumberFormat('en-US').format(refferred.TotalAmount)}
                      </TableCell>
                      <TableCell className="px-3 py-3 text-sm text-gray-700 dark:text-gray-300">
                        {refferred.SsdRefferredNumber == 0 ? "Pending" : `${new Intl.NumberFormat('en-US').format(refferred.SsdRefferredAmount)}`}
                      </TableCell>
                      <TableCell className="px-3 py-3 text-sm text-gray-700 dark:text-gray-300">
